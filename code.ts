...
async function applyTextWithShrink(
  ...
  const minScaleRaw =
    Number.isFinite(minScaleInput) && minScaleInput > 0 ? minScaleInput : 0.7;
...
async function runLocalization(settings: PluginSettings): Promise<void> {
  ...
  const layoutBounds = settings.keepDuplicates
    ...
    : null;
  const localeYOffset =
    settings.keepDuplicates && layoutBounds
...
async function getStoredSettings(): Promise<PluginSettings> {
  ...
  return {
    ...
    minShrinkScale:
      stored?.minShrinkScale === undefined ? 0.7 : stored.minShrinkScale,
  };
}
...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === "start") {
    ...
    const settings: PluginSettings = {
      ...
      minShrinkScale:
        typeof rawSettings.minShrinkScale === "number"
